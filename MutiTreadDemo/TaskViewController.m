...
-(void)p_setupViews{
    ...
    self.textView.text = @"主队列，同步添加多个任务\n会crash！！！！！\n原因如下：\n1、主队列在执行dispatch_sync函数,函数会把一个block加入到指定的队列，此函数要求执行完block才返回.\n2、dispatch_sync函数把block加入的指定队列，是主队列时。就会出现下面的状况：\n \n原主队列有A,B,C三个任务，在A任务完成后，执行B任务时，突然需要在B中嵌套一个D任务，按照先进先出的原则，现在主队列有未完成的任务B和待执行任务C,D，但是有一个要求，D要求它先执行完再执行其他的,此时我们可以发现队列中的任务产生了冲突，D等待主队列完成BC任务后执行，但是B完成任务的前提是D完成任务。造成死锁";
